...
if strcmp(inoutorxor,'i')
    ...
    clipped_inds = find( ((coords(:,1)>minXthresh) & (coords(:,1)<maxXthresh) &...
                        (coords(:,2)>minYthresh) & (coords(:,2)<maxYthresh)) == 1);
...
elseif strcmp(inoutorxor,'xor')
    ...
    clipped_coords = coords( xor( (coords(:,1)<=minXthresh) | (coords(:,1)>=maxXthresh) , ...
                               (coords(:,2)<=minYthresh) | (coords(:,2)>=maxYthresh) ) ,:);
    clipped_inds = find( xor( (coords(:,1)<=minXthresh) | (coords(:,1)>=maxXthresh) , ...
                               (coords(:,2)<=minYthresh) | (coords(:,2)>=maxYthresh) ) == 1);
